...
- **`src/lib/links.ts`** holds outbound destinations and the mailto CTA hrefs (`VALUECOMPASS_URL`, `BUILDWITHWHY_URL`, `DECISION_HREF`, `MAIL_HREF`, `EMAIL`). Every CTA points at one of these — change them there, not at the call sites.
